...
export const createUser = catchAsync(async (body) => {
  const { name, email, password, role, phone, phoneCode } = body;

  if (!name || !email || !password || !role || !phone || !phoneCode) {
    return {
      type: 'Error',
...
export const updateUserProfileImage = catchAsync(async (id, profileImage) => {
  ...
  const image = await uploadFile(
    dataUri(profileImage.buffer, extension).content,
    folderName,
    extension
  );
...
/**
 * Add User Profile Image Using It's ID
 * @param     {ObjectId}  id - User ID
 * @param     {Object}    profileImage - Profile Image
 * @returns   {Object<type|message|statusCode|user>}
 */
export const addUserProfileImage = catchAsync(async (id, profileImage) => {
  let user = await User.findById(id);
